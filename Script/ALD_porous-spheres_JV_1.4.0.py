...
Diffusion-reaction model for ALD on a porous sphere
Version: 1.4.0

This Python script provides the solution of the one-dimensional diffusion equation with surface reaction for volumetric reactant number density and surface coverage as function of time along the radial coordinate of a porous sphere (Eq. 1 of Heikkinen et al., Phys. Chem. Chem. Phys., 2024, 26, 7580, DOI: 10.1039/d3cp05639b, for spheres). For the surface reaction, the model uses Langmuir adsorption and includes a desorption term (as in Eq. 16 of Ylilammi et al., J. Appl. Phys. 123, 205301, 2018, DOI: 10.1063/1.5028178). The partial differential equations for diffusion and reaction are solved numerically. For the calculations, the effective diffusion coefficient is assumed to be constant along the porous sphere. This script can be used in a wide range of diffusion regimes (Kn number from Kn<<1 to Kn>>1).

DO NOT modify this script. Use/modify parameters in the file “Parameters_ALD_porous-sphere.xlsx”

@author: Dr. Jorge A. Velasco, by request of Prof. Riikka L. Puurunen (Catalysis Group, Aalto University)
...
wb =        load_workbook("Parameters_ALD_porous-sphere.xlsx", data_only=True)  
ws =        wb["Parameters"]
Runcode =   ws["B1"].value
...
c =         ws["B10"].value     # Sticking coefficient (-)
q0 =        ws["B11"].value     # Adsorption capacity (#/m2) 
Pd =        ws["B12"].value     # Desorption probability (s-1)
t_pulse =   ws["B13"].value     # Pulse time (s) 
T =         ws["B14"].value     # Temperature (K) 
pA0 =       ws["B15"].value     # Partial pressure of reactant at pellet surface (Pa) 
...
# ======== Calculated parameters using input parameters values ========

nA0 =       NAv*pA0/(R*T)               # Volumetric number density of reactant A (m−3)
vA =        np.sqrt((8*R*T)/(np.pi*MA)) # Average speed of A molecule (m/s)
DKn =       (1/3)*vA*dpore              # Knudsen diffusion (m2/s))
...
D1 =        (1/(1/DA + 1/DKn)) 
D =         (Por/Tau)*D1                # Effective diffusion, based on the model of Evans et al., J. Chem. Phys. 35, 2076–2083 (1961), https://doi.org/10.1063/1.1732211
...
k1 =        (1/4)*s_AP*vA*c             # for nA PDE equation, adsorption-related
kd =        s_AP*Pd*q0                  # for nA PDE equation, desorption-related
k2 =        (1/4)*(1/q0)*vA*c           # for theta PDE equation, adsorption-related
...
dr =        r[1] - r[0]
# Initial conditions
nA_init =   np.zeros(N)                 # for nA(r, 0)
theta_init = np.zeros(N)                # for theta(r, 0)
...
# ======== Estimations based on the PDE's solution ========

# ---- Estimation fraction of coated volume as function of "r" and metal loading -----
FracVol =       1-(r/Rp)**3
...
Metalmass =     Frac*(q0*MM*(SA*1000)/NAv)* msphere # 
MOxmass =       Metalmass*MMOx/MM
wtper =         Metalmass*100/(msphere+MOxmass)
wtfull =        (q0*MM*(SA*1000)/NAv)*msphere*100/(msphere+MOxmass)
...
plt.xlabel('Distance from surface, d=Rp-r (μm)')
plt.ylabel('Time (s)')
...
plt.colorbar(label='Surface coverage, θ(x,t)')
plt.xlabel('Distance from surface, d=Rp-r (μm)')
plt.ylabel('Time (s)')
plt.title('Temporal evolution of surface coverage, θ')
plt.show()

#------ Final profiles plots -------

plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.plot((Rp-r)*1e6, nA_sol[:, -1], label='nA at t_final')
plt.xlabel('Distance from surface, d=Rp-r (µm)')
plt.ylabel('nA (1/m^3)')
plt.title('Final nA profile (at t=t_pulse)')
plt.grid(True)
...
plt.xlabel('Distance from surface, d=Rp-r (µm)')
plt.ylabel('θ (-)')
plt.title('Final θ profile (at t=t_pulse)')
plt.grid(True)
...
plt.xlabel('Distance from surface, d=Rp-r (µm)')
plt.ylabel('pA (Pa)')
plt.title('Final pA profile (at t=t_pulse)')
plt.grid(True)
...
param_data = {
    "Parameter": ["Running Code", "Script's name & version:", "Radius of the sphere, R_p (m)", "Porosity of the sphere, ε (-)", "Tortuosity of the sphere, τ (-)", "Pore diameter d_pore (m)", "Specific surface area, S (m2/g)", "Pore volume, V_pore (m3/g)", "Skeletal density of sphere, ρ_S (kg/m3)", "Sticking coefficient, c (-)", "Adsorption Capacity, q0 (#/m2)", "Desorption probability, P_d (1/s)", "Pulse time, t_pulse (s)", "Temperature, T (K)", "Partial pressure of reactant at surface, p_A0 (Pa)", "Partial pressure of inert gas, p_I (Pa)", "Molar mass of reactant A, M_A (kg/mol)", "Molar mass of inert, M_I (kg/mol)","Molecular diameter of reactant A, d_A (m)","Molecular diameter of inert, d_I (m)", "Atomic mass of metal, M_M (kg/mol)", "Molar mass of metal oxide deposited, M_MOx (kg/mol)","Number of spatial grid points in r-axis", "Number of time grid points"],
    "Value": [Runcode, ScriptN, Rp, Por, Tau, dpore, SA, VP, DenS, c, q0, Pd, t_pulse, T, pA0, pI, MA, MI, dA, dI, MM, MMOx, N, Ntime]
}
param_df = pd.DataFrame(param_data)

# Calculated parameter values
calcvalues_data = {
    "Calculated Values": ["Volumetric reactant number density, n_A0 (1/m3)", "Thermal velocity of molecule A, v_A (m/s)", "Knudsen diffusion coefficient, D_Kn (m2/s)", "Molecular diffusion coefficient, D_A (m2/s)", "Effective diffusion coefficient, D_eff (m2/s)", "Aspect ratio as R_p/d_pore, AR (-)", "Knudsen number, Kn (-)", "Thiele modulus, h_T", "Exposure, P_A0*t (Pa-s)", "Metal weight percent, w (%)", "Vol.fraction coated, V_coated/V_sphere (%)", "Max. (possible) metal weight percent, w_max (%)", "Penetration depth (Rp-r) at theta=0.5, d_50% (m)", "Normalized Penetration depth (R_p-r/R_p) at theta=0.5, (d/R_p)_50% (-)", "Slope 'theta/(R_p-r/R_p)' at theta=0.5, (-)", "Diff. between the two data points (y-axis) around theta=0.5 (%)"],
    "Value": [nA0, vA, DKn, DA, D, Rp/dpore, Knudsen, Thiele, pA0*t_pulse, wtper, Frac*100, wtfull, PD50, PD50rel, slopePD50rel, ThetaDiff]
}
calcvalues_df = pd.DataFrame(calcvalues_data)

# Final profiles data (Rp-r, pA, theta, nA, Vol. coated fraction)
...
with pd.ExcelWriter(filename1, engine="openpyxl") as writer:
    ...
    calcvalues_df.to_excel(writer, sheet_name="Results", index=False, startrow=26, startcol=0)
    data_df.to_excel(writer, sheet_name="Results", index=False, startrow=0, startcol=4)

# ---- Figure of a cross-sectional section ---------------
...
ax.set_title('Radial Profile of θ (at t=t_pulse)')
plt.tight_layout()
plt.show()
